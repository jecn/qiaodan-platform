package org.safari.platform.common.dao;
...
import org.safari.platform.common.filter.PropertyFilter.MatchType;
...
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> {
	 ...
	 * 分页获取全部对象.
	 */
	public PageUtil<T> findAll(final PageUtil<T> page) {
	...
	public PageUtil<T> findPage(final PageUtil<T> page, final String hql, final Object... values) {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public PageUtil<T> findPage(final PageUtil<T> page, final Criterion... criterions) {
		Assert.notNull(page, "page不能为空");
	 ...
	 * 设置分页参数到Query对象,辅助函数.
	 */
	protected Query setPageParameterToQuery(final Query q, final PageUtil<T> page) {
	 ...
	 */
	protected Criteria setPageParameterToCriteria(final Criteria c, final PageUtil<T> page) {
		...
		/*if (page.isOrderBySetted()) {
			String[] orderByArray = StringUtil.split(page.getOrderBy(), ',');
			String[] orderArray = StringUtil.split(page.getOrder(), ',');

			Assert.isTrue(orderByArray.length == orderArray.length, "分页多重排序参数中,排序字段与排序方向的个数不相等");

			for (int i = 0; i < orderByArray.length; i++) {
				if (Global.ASC.equals(orderArray[i])) {
