...
function loadGrid(){
	...
	$("#mainTable").datagrid({    
			...
			columns: [[
			           ...
			           {field:'link',title:'链接',sortable:false},
...
function view(){
	...
	top.getDlg("${ctxAdmin}/shoes/recm/view/" + row.id ,"dlgb","查看",520,206);
}

//新增
function add(){
	top.getDlg("${ctxAdmin}/shoes/recm/create/" ,"dlgb","新增",520,360);
}
...
function update(){
	...
	top.getDlg("${ctxAdmin}/shoes/recm/update/" + row.id ,"dlgb","修改",520,360);
}
...
function del(){
	...
	$.messager.confirm("删除确认", "您确定要删除选择的记录吗？", function(data){
		if (data){
			$.ajax({
				type:"get",
				url:"${ctxAdmin}/shoes/recm/delete/"+row.id,
...
function opt(id,useable){
	...
	$.messager.confirm(op + "确认", "您确定要" + op +  "该字典类型吗？", function(data){
		if (data){
			$.ajax({
				type:"get",
				url:"${ctxAdmin}/shoes/recm/"+id +"/opt?useable="+useable,
				success: function(data){
					if(data == "success"){
						$("#mainTable").datagrid("reload");	// reload the user data
						$("#mainTable").datagrid("clearSelections"); //clear selected options
