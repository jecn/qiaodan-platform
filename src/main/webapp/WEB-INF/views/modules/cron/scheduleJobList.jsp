    ...
    onSelect:function(title,index){    
    	if(index==1){
    		dg_running=$('#dg_running').datagrid({    
    		    url:'${ctx}/sys/scheduleJob/running/json', 
    		    fit : true,
    			fitColumns : true,
    			border : false,
    			idField : 'id',
...
function stop(){
	...
	parent.$.messager.confirm('提示', '确定要暂停任务？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/sys/scheduleJob/"+row.name+"/"+row.group+"/stop",
...
function resume(){
	...
	parent.$.messager.confirm('提示', '确定要恢复任务？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/sys/scheduleJob/"+row.name+"/"+row.group+"/resume",
				success: function(data){
					if(data=='success'){
						dg.datagrid('reload');
						parent.$.messager.show({ title : "提示",msg: "操作成功！", position: "bottomRight" });
...
function del(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/sys/scheduleJob/"+row.name+"/"+row.group+"/delete",
				success: function(data){
					if(data=='success'){
						dg.datagrid('reload');
						parent.$.messager.show({ title : "提示",msg: "操作成功！", position: "bottomRight" });
...
function sureUpd(row){
	parent.$.messager.confirm('提示', '确定要修改任务？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/sys/scheduleJob/"+row.name+"/"+row.group+"/update",
...
function startNow(){
	...
	parent.$.messager.confirm('提示', '确定要立即运行一次该任务？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/sys/scheduleJob/"+row.name+"/"+row.group+"/startNow",
				success: function(data){
					if(data=='success'){
						//dg.datagrid('reload');
						parent.$.messager.show({ title : "提示",msg: "操作成功！", position: "bottomRight" });
